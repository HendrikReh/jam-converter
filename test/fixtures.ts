...
export const rawNode = (
  ...
  type,
  name,
  ...(type === 'DOCUMENT' ? {} : { parentIndex: { guid: guid(parent), position: String(id) } }),
  visible: true,
  ...extra,
});
export function jamBytes(nodes: any[]) {
  ...
  const canvas = assembleCanvasFig({
    ...
    messageCompressed: zstdCompressSync(parts.messageRaw),
    passThrough: [],
  });
  return zipSync({
    'canvas.fig': canvas,
    'meta.json': strToU8(JSON.stringify({ file_name: 'Demo' })),
  });
}
export const simpleNodes = () => [
  rawNode(0, 'DOCUMENT', 'Document'),
  rawNode(1, 'SECTION', 'Commerce IST'),
  rawNode(2, 'SHAPE_WITH_TEXT', 'Shop', 1, {
    nodeGenerationData: { overrides: [{ textData: { characters: 'Shop' } }] },
  }),
  rawNode(3, 'SHAPE_WITH_TEXT', 'ERP', 1, { textData: { characters: 'ERP' } }),
  rawNode(4, 'CONNECTOR', 'orders', 1, {
    connectorStart: { endpointNodeID: guid(2) },
    connectorEnd: { endpointNodeID: guid(3) },
    connectorStartCap: 'ROUND',
